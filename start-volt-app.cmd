...
rem icon so every process reads "Volt". electron.exe is left untouched so
rem npx and electron-builder keep working. Packaged builds already do this.
set "VOLT_EXE=node_modules\electron\dist\Volt.exe"
set "RCEDIT=node_modules\electron-winstaller\vendor\rcedit.exe"
if not exist "%VOLT_EXE%" (
  echo Preparing the Volt runtime...
  copy /Y "node_modules\electron\dist\electron.exe" "%VOLT_EXE%" >nul 2>nul
  if exist "%RCEDIT%" (
