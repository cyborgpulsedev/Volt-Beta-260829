...
(function (global) {
  ...
  Volt.Voice = {
    ...
    _watchSettingsModal() {
      ...
      // engine pickers must react LIVE — without these listeners, choosing
      // "Custom endpoint" showed nothing until the modal was reopened, which
      // read as "selecting it doesn't do anything"
      const eng = $("set-tts-engine"), stt = $("set-stt-engine");
      if (eng) eng.addEventListener("change", () => this._syncEngineVisibility());
      if (stt) stt.addEventListener("change", () => this._syncEngineVisibility());
      // system voices often finish loading AFTER the modal first opens —
      // refresh the voice list whenever the platform announces them
    ...
    async _refreshSettingsFields() {
      ...
      if (voice) {
        ...
        if (!list.length) voice.innerHTML = '<option value="">No system voices found — try External endpoint</option>';
        for (const v of list) {
      ...
      if (test) test.onclick = () => {
        // adopt the form's current engine/URL/key first — otherwise testing an
        // external endpoint exercises the previously saved settings, which
        // reads as the button doing nothing
        this._saveSettings();
        Promise.resolve(this.speak("This is the voice Volt will read with."))
          .then((ok) => {
            this._app().toast(ok ? "Speaking…" : "No speech engine available — check your Voice settings", ok ? "ok" : "error");
          })
          .catch((e) => this._app().toast(String((e && e.message) || e), "error"));
      };
    },
    ...
    _syncEngineVisibility() {
      ...
      // the system-voice picker only applies to the built-in engine
      const vf = $("tts-voice-field");
      if (eng && vf) vf.hidden = eng.value !== "web";
    ...
    async voices() {
      ...
      // Voices load asynchronously and, on Windows, sometimes well after the
      // first getVoices() call (which is what triggers the load). One 1.5s
      // wait routinely missed them — poll for up to ~6s, resolving early on
      // the voiceschanged event or the first non-empty poll.
      await new Promise((res) => {
        ...
        const tick = () => {
          if (settled) return;
          if ((synth.getVoices() || []).length || ++tries >= 12) return done();
          setTimeout(tick, 500);
        };
        tick();
      });
