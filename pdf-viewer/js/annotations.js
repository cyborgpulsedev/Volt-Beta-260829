...
(function (global) {
  ...
  Volt.Ann = {
    ...
    _endDrag(ev, wrap) {
      ...
      // a CLICK is small in BOTH axes. With `||` a straight drag along one
      // line of text (660px wide, <3px tall — the commonest highlight
      // gesture there is) was classed as a click and silently did nothing.
      if (x2 - x1 < 3 && y2 - y1 < 3) {
    ...
    _spansInRect(wrap, x1, y1, x2, y2, thin = false) {
      ...
      for (const s of spans) {
        ...
        // `thin`: a straight drag along ONE line has no height, so no line's
        // center can fall inside the band and the center test selects
        // nothing. Fall back to the line whose BOX the drag passes through —
        // boxes overlap by only a pixel or two, so it stays unambiguous.
        if (sx2 < x1 || sx1 > x2) continue;
        if (thin ? (y1 < sy1 || y1 > sy2) : (cy < y1 || cy > y2)) continue;
    ...
    _selectionLines(wrap, sx, sy, ex, ey) {
      ...
      const spans = this._spansInRect(wrap, x1, y1, x2, y2, (y2 - y1) < 3);
    ...
    /** Did this PDF store its objects in object streams? Matching the source
        keeps a saved file the size it was; a classic-only rewrite inflates
        every modern PDF. Scanning the bytes is enough — an /ObjStm can only
        appear as a stream dictionary type. */
    _usesObjectStreams(bytes) {
      try {
        const u8 = bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes);
        // "/ObjStm" as bytes: 47 79 98 106 83 116 109
        const pat = [47, 79, 98, 106, 83, 116, 109];
        outer: for (let i = 0; i <= u8.length - pat.length; i++) {
          for (let j = 0; j < pat.length; j++) if (u8[i + j] !== pat[j]) continue outer;
          return true;
        }
        return false;
      } catch (e) { return false; } // unreadable → classic, the safe form
    },

    /** Burn the annotations into the open document and return the PDF bytes.
        `opts.classic` forces the classic xref / no-object-stream form that
    ...
    async toAnnotatedPdf(opts = {}) {
      ...
      // Classic output (no object streams / xref streams) is REQUIRED by
      // Volt.Secure.lock and Volt.Sign: both rebuild the xref + trailer
      // byte-level and need every object as a top-level `N G obj` with a
      // classic table — objects hidden in an ObjStm behind an xref stream
      // would produce a locked file whose /Root is unreachable and its
      // password check broken. Everything else matches the SOURCE document:
      // forcing classic on a file that used object streams rewrote it ~12%
      // larger for no benefit the user asked for.
      const srcUsesObjStm = this._usesObjectStreams(app.currentDocBytes);
      const useObjectStreams = opts.classic ? false : srcUsesObjStm;
    ...
    async buildEditedPdf(plan) {
      ...
      // pdf-lib deduplicates shared indirect objects — fonts, images, XObjects
      // — WITHIN a single copyPages call, and only there. Copying page by page
      // therefore re-copied every shared resource once per page: reordering two
      // pages of a 4.7 MB / 207-page document produced a 23 MB file. Batch each
      // source into ONE call, then place the results in plan order; the rebuilt
      // file now lands at the size of the original (and builds ~40% faster).
      const otherDocs = new Map();      // bytes → loaded PDFDocument
      const srcIdx = [];                // 0-based pages to take from the open document
      const otherIdx = new Map();       // bytes → 0-based pages to take from that file
      for (const e of plan) {
        if (e.kind === "doc") srcIdx.push(e.oldPage - 1);
        else if (e.kind === "other") {
          if (!otherIdx.has(e.bytes)) otherIdx.set(e.bytes, []);
          otherIdx.get(e.bytes).push(e.page - 1);
        }
      }
      const srcPages = srcIdx.length ? await out.copyPages(src, srcIdx) : [];
      const otherPages = new Map();
      for (const [bytes, idx] of otherIdx) {
        let od = otherDocs.get(bytes);
        if (!od) {
          od = await PDFDocument.load(bytes.slice(0), { ignoreEncryption: true });
          otherDocs.set(bytes, od);
        }
        otherPages.set(bytes, await out.copyPages(od, idx));
      }

      // place the copied pages in plan order (each queue is consumed in the
      // same order its indices were collected, so page N lands where it belongs)
      let srcNext = 0;
      const otherNext = new Map();
      for (const e of plan) {
        let page;
        if (e.kind === "doc") {
          page = out.addPage(srcPages[srcNext++]);
        } else if (e.kind === "blank") {
          page = out.addPage([e.w, e.h]);
        } else if (e.kind === "other") {
          const i = otherNext.get(e.bytes) || 0;
          otherNext.set(e.bytes, i + 1);
          page = out.addPage(otherPages.get(e.bytes)[i]);
        } else {
