...
(function (global) {
  ...
  Volt.App = {
    ...
    renderGen: 0,           // bumped whenever every page is torn down (zoom, rotate,
                            // view-mode, new doc) — an in-flight _ensurePage that
                            // wakes up on an older generation discards its own wrap
                            // instead of leaving a stale-size duplicate in the DOM
    ...
    async init() {
      ...
      this.elements = {
        ...
        btnThemeNeon: $("btn-theme-neon"), btnThemePaper: $("btn-theme-paper"), btnThemeQuiet: $("btn-theme-quiet"),
        ...
        btnReadaloud: $("btn-readaloud"), toolsEmptyHint: $("tools-empty-hint"),
        ...
        aiModelRefresh: $("ai-model-refresh"),
        ...
        tierDeviceNote: $("tier-device-note"),
    ...
    _wireToolbar() {
      ...
      // skin: View ▸ Neon / Paper / Quiet — chrome AND document theme
      if (el.btnThemeNeon) el.btnThemeNeon.addEventListener("click", () => this._setTheme("neon"));
      if (el.btnThemePaper) el.btnThemePaper.addEventListener("click", () => this._setTheme("paper"));
      if (el.btnThemeQuiet) el.btnThemeQuiet.addEventListener("click", () => this._setTheme("quiet"));
    ...
    /* ── skin (View ▸ Neon / Paper / Quiet) ───────────────────
       The theme is applied via body[data-theme] (CSS overrides the accent
       palette) and persisted per user in localStorage. The View menu items
       show a ✓ on the active skin. Legacy "dark"/"light" values from older
       builds migrate to Neon. */
    _theme() {
      try {
        const t = localStorage.getItem("volt:theme");
        return (t === "paper" || t === "quiet") ? t : "neon";
      } catch (e) { return "neon"; }
    },
    ...
    _applyTheme(t) {
      ...
      const el = this.elements;
      if (el.btnThemeNeon) el.btnThemeNeon.classList.toggle("checked", t === "neon");
      if (el.btnThemePaper) el.btnThemePaper.classList.toggle("checked", t === "paper");
      if (el.btnThemeQuiet) el.btnThemeQuiet.classList.toggle("checked", t === "quiet");
    },

    _setTheme(t) {
      const v = (t === "paper" || t === "quiet") ? t : "neon";
      try { localStorage.setItem("volt:theme", v); } catch (e) { /* ignore */ }
      this._applyTheme(v);
      this.toast(v === "quiet" ? "Quiet — glow muted for calmer reading"
        : v === "paper" ? "Paper — light background, neon accents"
        : "Neon — full cyan glow restored", "ok");
    },
    ...
    /* ── Send feedback (Volt ▾ → Send feedback…) ──────────────
       ...
       window.open), where the user reviews and submits it. When GitHub can't
       open (offline, unreachable, or the external open fails), the draft is
       saved as a local .md file instead so the report is never lost. */
    _openFeedback() {
      const el = this.elements;
    ...
    async _sendFeedback() {
      ...
      const body = text + "\n\n---\n\n" + env.join("\n") + (email ? "\n\n**Contact:** " + email : "");
      const url = "https://github.com/cyborgpulsedev/Volt-Beta-260829/issues/new?labels=" +
      ...
      // Offline / local fallback: when GitHub can't open (machine offline,
      // github.com unreachable, or the external-browser open fails), save the
      // drafted report as a file instead — the modal submit must never lose
      // the report. The file keeps the prefilled GitHub URL so the user can
      // still submit it later from any machine.
      const stamp = new Date();
      const pad = (n) => String(n).padStart(2, "0");
      const filename = "volt-feedback-" + stamp.getFullYear() + pad(stamp.getMonth() + 1) + pad(stamp.getDate()) +
        "-" + pad(stamp.getHours()) + pad(stamp.getMinutes()) + pad(stamp.getSeconds()) + ".md";
      const report =
        "# " + title + "\n\n" +
        "- **Type:** " + type + "\n" +
        "- **Filed:** " + stamp.toISOString() + "\n\n" +
        body + "\n\n---\n\n" +
        "*This draft was saved locally because GitHub couldn't be reached. To submit it later, open:\n" +
        url + "*";
      const saveLocal = (why) => {
        Utils.download(new Blob([report], { type: "text/markdown;charset=utf-8" }), filename);
        this.toast(why + " — feedback saved as " + filename, "ok");
      };
      // fast path: the machine reports itself offline — don't even probe
      if (navigator.onLine === false) { saveLocal("Offline"); return; }
      // short reachability probe. A no-cors fetch to github.com rejects only
      // on a network-level failure (DNS/connect/abort) — any HTTP status is an
      // opaque success — so this is a safe, CORS-free liveness check.
      let reachable = true;
      try {
        const ctl = new AbortController();
        const timer = setTimeout(() => ctl.abort(), 2000);
        await fetch("https://github.com", { mode: "no-cors", cache: "no-store", signal: ctl.signal });
        clearTimeout(timer);
      } catch (e) { reachable = false; }
      if (!reachable) { saveLocal("Can't reach GitHub"); return; }
      try {
        await this._openExternal(url);
        this.toast("Feedback draft opened — review & submit on GitHub", "ok");
      } catch (e) {
        saveLocal("Couldn't open GitHub");
      }
    },

    /** Open an external URL in the default browser (desktop: shell.openExternal
        via the preload bridge; PWA: window.open). A small overridable seam so
        tests can exercise the open-failure fallback without touching the
        frozen contextBridge object. */
    ...
    _wireModals() {
      ...
      // backdrop click closes — every modal, for the same reason _modals()
    ...
    /* ── modal focus management ───────────────────────────────
       ...
       the opener on close. */
    /* Every .modal in the document, read from the DOM rather than a list kept
       by hand. The hand-kept list named 8 of the 15 modals, so Secure, Sign,
       Setup, Feedback, Signature, Form field and the file:// notice got no
       focus trap, no inert background, no one-modal-at-a-time guard, and no
       Escape — Escape fell through to the annotation-mode reset instead, so a
       dialog opened with a markup tool active could not be dismissed with the
       keyboard at all. Deriving the list means a new modal can never be
       forgotten again. */
    _modals() {
    ...
    _wireSetupWizard() {
      ...
      if (el.startHint) {
        el.startHint.addEventListener("click", (e) => { e.stopPropagation(); this._startHintClick(); });
      }
      el.setupSkip.addEventListener("click", () => {
        // skip = abandon: undo any color-scheme preview back to the saved theme
        let saved = "neon";
        try { saved = localStorage.getItem("volt:theme") || "neon"; } catch (e) { /* default */ }
        this._applyTheme(saved);
        this._markSetupDone(false);
        this._closeModal(el.setupModal);
      });
      // color scheme radios preview LIVE — the whole app restyles behind the
      // wizard so the choice is visible before committing (Finish persists it)
      el.setupModal.querySelectorAll("input[name=setup-skin]").forEach((r) => {
        r.addEventListener("change", () => { if (r.checked) this._applyTheme(r.value); });
      });
    ...
    /** First-run hint (replaces the old setup-banner popup): a pill sits as a
        normal flex child next to the Volt logo. On exit the CSS transition
        shrinks its max-width / margin / padding to zero; the toolbar's flex
        layout redistributes the freed space so View and the search bar expand
        rightward without any overlap. The pill stays in-flow the whole time.
    ...
    _maybeShowStartHint(force) {
      ...
      // entrance: add .entering (shrunk state), then remove it on the next
      // frame so the CSS transition grows the pill in
      hint.classList.add("entering");
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          hint.classList.remove("entering");
        });
      });
      const leave = () => {
        ...
        hint.addEventListener("transitionend", function handler(e) {
        ...
        // safety: if transitionend never fires (rare Chromium bug), hide
        // after the transition duration anyway
        this._startHintHideTimer = setTimeout(() => {
          if (!hint.hidden) { hint.hidden = true; hint.classList.remove("exiting"); delete hint.dataset.playing; }
        }, 1200);
      };
      this._startHintTimer = setTimeout(leave, 480 + 480 + 3400);
    },
    _startHintClick() {
      const hint = this.elements.startHint;
      if (this._startHintTimer) { clearTimeout(this._startHintTimer); this._startHintTimer = null; }
      if (this._startHintHideTimer) { clearTimeout(this._startHintHideTimer); this._startHintHideTimer = null; }
      if (hint) {
        hint.getAnimations().forEach((a) => a.cancel());
        hint.hidden = true;
        hint.classList.remove("entering", "exiting");
        delete hint.dataset.playing;
      }
      this._markSetupDone(false); // engaged — the hint won't replay on launch
      // Open the Volt logo menu and focus the setup wizard item
      const brandBtn = document.getElementById("btn-brand");
      if (brandBtn) brandBtn.click();
      setTimeout(() => {
        const setupItem = document.getElementById("btn-menu-setup");
        if (setupItem) setupItem.focus();
      }, 80);
    },
    _markSetupDone(done) {
    ...
    _setupShow(step) {
      ...
      const segs = [...el.setupSteps.querySelectorAll(".setup-track-seg")];
      segs.forEach((d, i) => {
    ...
    async _setupAiStatus() {
      ...
      // tier expectations: mark what this machine's RAM fits and what's
      // already pulled, and offer the sized one-click install right here
      const tiers = document.getElementById("setup-tiers");
      const install = document.getElementById("setup-ai-install");
      const note = document.getElementById("setup-ai-install-note");
      if (tiers && AI._recommendedTier) {
        const reco = AI._recommendedTier();
        const installed = new Set((AI._bootstrap && AI._bootstrap.models) || []);
        for (const card of tiers.querySelectorAll(".setup-tier")) {
          card.classList.toggle("reco", card.dataset.tier === reco.model);
          card.classList.toggle("installed", installed.has(card.dataset.tier));
        }
        if (install && note) {
          const gb = AI._deviceMemGB ? AI._deviceMemGB() : null;
          if (installed.has(reco.model) || model) {
            install.hidden = true;
            note.hidden = true;
          } else {
            install.hidden = false;
            install.textContent = "Install " + reco.model + (gb ? " — sized for this machine (~" + gb + " GB RAM)" : "");
            note.hidden = true;
            install.onclick = () => {
              install.disabled = true;
              note.hidden = false;
              note.textContent = "Installing — progress shows here and in the AI panel…";
              Promise.resolve(AI._bootstrapPrimary()).then(() => {
                install.disabled = false;
                this._setupAiStatus(); // re-render: ready / installed states
              }).catch(() => { install.disabled = false; });
            };
          }
          if (AI._deviceIsWeak && AI._deviceIsWeak()) {
            note.hidden = false;
            note.textContent = AI._weakDeviceWarning();
          }
        }
      }
    },
    ...
    async _setupFinish() {
      ...
      const skinVal = skin ? skin.value : "neon";
      if (this._setTheme) this._setTheme(skinVal);
      lines.push("Skin — " + (skinVal === "quiet" ? "Quiet" : skinVal === "paper" ? "Paper" : "Neon"));
    ...
    _docReady() {
      ...
      // the rasterized page images belong to the OUTGOING document — the key
      // is "d:<page>", so without this every thumbnail in the new document
      // blits the old one's page (sidebar rail AND pages manager share it)
      this._pageThumbCache.clear();
      ...
      // Tools menu hint: show "open a document" when no tool items are visible
      if (this.elements.toolsEmptyHint) {
        const anyToolVisible = (this.elements.btnOcr && !this.elements.btnOcr.hidden) || (this.elements.btnReadaloud && !this.elements.btnReadaloud.hidden);
    ...
    _layoutPages() {
      ...
      // the page readout is computed from pageLayout, which only exists now —
      // without this every freshly opened document showed "-" where "p.1 / N"
      // belongs until the user happened to scroll
      this._updateStatus();
    },
    ...
    async _ensurePage(pageNum) {
      ...
      // the teardown generation this render belongs to; if it changes while we
      // await, everything below is stale — bail and take the wrap with us
      const gen = this.renderGen;
      const stale = (wrap) => {
        if (this.renderGen === gen) return false;
        if (wrap) wrap.remove();
      ...
      try {
        ...
        // a teardown during the render makes this wrap the wrong size for the
        // current zoom — drop it rather than registering it (the check must
        // come BEFORE rendered.set, or the stale entry replaces the good one)
        if (stale(wrap)) return;
        ...
        if (stale(wrap)) { this.rendered.delete(pageNum); return; }
        if (!wrap.isConnected) { this.pendingRender.delete(pageNum); this.rendered.delete(pageNum); return; }
    ...
    _insertWrap(wrap, pageNum) {
      // One page, one wrap — always. Overlapping teardowns (a view-mode flip
      // that re-enters through _applyFitZoom, say) can clear pendingRender
      // while an earlier render still holds an un-registered wrap in the DOM,
      // and a second render for the same page then appends another. Evicting
      // any prior wrap for this page here makes insertion idempotent, so no
      // interleaving can leave the page duplicated on screen.
      for (const prev of [...this.elements.pages.querySelectorAll(".page-wrap")]) {
        if (prev !== wrap && Number(prev.dataset.page) === pageNum) prev.remove();
      }
      // insert in order
      let inserted = false;
      for (const [n, r] of this.rendered) {
    ...
    /** Tear every page down and re-render at the current scale/rotation/mode.
        Shared by zoom, view-mode and restore-view — each used to inline this
        loop, and each inlined copy walked ONLY `this.rendered`. A page still
        inside _ensurePage has its wrap in the DOM but no `rendered` entry yet,
        so those loops skipped it: the stale-size wrap stayed on screen, the
        re-render appended a second wrap for the same page, and the document
        appeared to repeat pages at different sizes. Bumping renderGen makes
        the in-flight render discard its own wrap, and the DOM sweep clears
        any orphan that slipped through. */
    _resetPages() {
      this.renderGen++;
      for (const [n, r] of [...this.rendered]) {
        r.wrap.style.width = "";
      ...
      // sweep orphans: wraps in the DOM that no `rendered` entry owns
      for (const w of [...this.elements.pages.querySelectorAll(".page-wrap")]) {
        const n = Number(w.dataset.page);
        const owner = this.rendered.get(n);
        if (!owner || owner.wrap !== w) w.remove();
      }
      this.pendingRender.clear();
      this._renderVisible();
    },
    ...
    _setZoom(z, keepFocus) {
      ...
      this._layoutPages();
      this._resetPages(); // re-render visible pages at the new scale
      this._updateZoomLabel();
    ...
    async _doExport(kind) {
      ...
      try {
        if (kind === "pdf") {
          ...
          el.signPass.value = "";
          try { el.signTsa.value = localStorage.getItem("volt:tsa-url") || ""; } catch (e) { el.signTsa.value = ""; }
          el.signPass.focus();
        } else if (kind === "pdfa") {
    ...
    async _doSignExport() {
      ...
      try {
        ...
        const tsaUrl = el.signTsa.value.trim();
        if (tsaUrl) { try { localStorage.setItem("volt:tsa-url", tsaUrl); } catch (e) { /* private mode */ } }
        this._closeModal(el.signModal);
        this.toast("Signing PDF…" + (tsaUrl ? " (timestamping with " + tsaUrl + ")" : ""), "ok");
        const bytes = await Volt.Ann.toAnnotatedPdf({ classic: true });
        ...
        this.toast("Digitally signed PDF exported — certificate: " + certName + (tsaUrl ? " · RFC 3161 timestamped" : ""), "ok");
      } catch (e) {
        this._openModal(el.signModal);
    ...
    _wireKeyboard() {
      window.addEventListener("keydown", (e) => {
        ...
        if (mod && !e.altKey) {
          ...
          if (k === "j" && e.shiftKey) { e.preventDefault(); if (Volt.AI) Volt.AI.askSelection(); return; }
