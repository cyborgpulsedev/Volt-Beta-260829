...
(function (global) {
  ...
  const _d = {
    ...
    genTime(d) { // RFC 3161 GeneralizedTime: YYYYMMDDHHMMSSZ (tag 0x18)
      const p = (n) => String(n).padStart(2, "0");
      const s = p(d.getUTCFullYear()) + p(d.getUTCMonth() + 1) + p(d.getUTCDate()) +
        p(d.getUTCHours()) + p(d.getUTCMinutes()) + p(d.getUTCSeconds()) + "Z";
      return this.tlv(0x18, [...s].map((c) => c.charCodeAt(0)));
    },
    utcTime(d) {
      const p = (n) => String(n).padStart(2, "0");
  ...
  /** RFC 7292 §B.2 key/IV derivation. hash: "SHA-1" (u = 20) or "SHA-256"
      (u = 32); both use v = 64. OpenSSL 3 exports default to a SHA-256 MAC,
      so the classic SHA-1-only derivation rejects real PFXs. */
  async function _pkcs12Kdf(subtle, password, salt, iterations, n, id, hash) {
    const h = hash || "SHA-1";
    const u = h === "SHA-256" ? 32 : 20, v = 64;
    ...
    const digest = async (bytes) => new Uint8Array(await subtle.digest(h, bytes));
    // 4. c = ceil(n / u)
  ...
  const Sign = {
    ...
    async parsePfx(pfxBytes, password) {
      ...
      if (macData) {
        ...
        // the MAC digest algorithm: RFC 7292 defaults to hmacWithSHA1, but
        // OpenSSL 3 / modern exporters use hmacWithSHA256 — read the OID and
        // derive with the matching hash (SHA-256: u = 32)
        const macAlgParts = _d.children(di[0].value, 0, di[0].value.length);
        const macAlgOid = _oidStr(macAlgParts[0].value);
        // accept both the RFC 7292 hmacWith* OIDs and the bare digest OIDs
        // (OpenSSL 3 writes `sha256` / `sha1` here)
        const macHash = macAlgOid === OID.hmacWithSHA256 || macAlgOid === "2.16.840.1.101.3.4.2.1"
          ? "SHA-256"
          : macAlgOid === OID.hmacWithSHA1 || macAlgOid === "1.3.14.3.2.26" ? "SHA-1" : null;
        if (!macHash) throw new Error("Unsupported PFX MAC algorithm: " + macAlgOid);
        const macDigest = di[1]; // OCTET STRING
        const salt = macParts[1].value;
        const iter = this._intOf(macParts[2].value);
        const key = await _pkcs12Kdf(subtle, password, salt, iter, macHash === "SHA-256" ? 32 : 20, 3, macHash);
        const hk = await subtle.importKey("raw", key, { name: "HMAC", hash: macHash }, false, ["sign"]);
    ...
    async _decryptSafeBag(subtle, algIdTlv, encrypted, password) {
      ...
      if (oid === OID.pbes2) {
        ...
        // PBKDF2-params: salt, iterationCount, keyLength (OPTIONAL), prf
        // (OPTIONAL, DEFAULT hmacWithSHA1). The prf is the AlgorithmIdentifier
        // SEQUENCE — find it by tag, since keyLength may sit before it.
        const prfTlv = kpp.find((k) => k.tag === 0x30);
        const prf = prfTlv ? _oidStr(_d.children(prfTlv.value, 0, prfTlv.value.length)[0].value) : null;
        ...
        // password → PBKDF2 bytes: exporters disagree. Legacy + Windows
        // (CAPI) use the PKCS#12 B.1 UTF-16BE + 0x0000 terminator; OpenSSL 3
        // uses the RAW password bytes. The MAC check already proved the
        // password correct, so this is an encoding-convention fallback, not
        // a password retry — try UTF-16BE first, then raw UTF-8, and use
        // whichever decrypts with valid padding.
        const pwVariants = [
          _pwBytes(password),
          new TextEncoder().encode(String(password || "")),
        ];
        let lastErr = null;
        for (const pwBytes of pwVariants) {
          try {
            const pwKey = await subtle.importKey("raw", pwBytes, { name: "PBKDF2" }, false, ["deriveBits"]);
    ...
    /* ── RFC 3161 timestamps ─────────────────────────────────── */
    /** Build a TimeStampReq (RFC 3161 §2.4.1): version 1, a SHA-256
        messageImprint over the given bytes, certReq = TRUE (so the reply
        carries the TSA certificate) and a random nonce. Returns the DER
        request bytes. */
    buildTimestampRequest(messageImprint) {
      const nonce = new Uint8Array(8);
      if (global.crypto && global.crypto.getRandomValues) global.crypto.getRandomValues(nonce);
      else for (let i = 0; i < 8; i++) nonce[i] = Math.floor(Math.random() * 256);
      const msgImprint = _d.seq(_d.alg(OID.sha256), _d.oct(messageImprint));
      // SEQ { version, messageImprint, nonce, certReq TRUE } — RFC 3161 field
      // order matters: a TSA rejects a request whose optional fields are out
      // of sequence (nonce comes BEFORE certReq)
      return new Uint8Array(_d.seq(_d.int([1]), msgImprint, _d.int([...nonce]), _d.tlv(0x01, [0xff])));
    },

    /** Parse an RFC 3161 TimeStampResp. Returns the TimeStampToken — the
        raw ContentInfo DER bytes — or throws with the TSA's status string.
        Accepts both the bare ContentInfo form OpenSSL emits and the
        `[0] IMPLICIT`-wrapped form some stacks write. */
    parseTimestampResponse(bytes) {
      const resp = _d.read(bytes, 0);
      const parts = _d.children(resp.value, 0, resp.value.length);
      const status = parts[0]; // PKIStatusInfo SEQUENCE
      const statusKids = _d.children(status.value, 0, status.value.length);
      const statusInt = this._intOf(statusKids[0].value);
      let statusString = "";
      const statusStr = statusKids[1]; // SET OF UTF8String (optional)
      if (statusStr && statusStr.tag === 0x31) {
        for (const s of _d.children(statusStr.value, 0, statusStr.value.length)) {
          statusString += String.fromCharCode.apply(null, s.value);
        }
      }
      if (statusInt !== 0 && statusInt !== 1) { // granted / grantedWithMods
        throw new Error("TSA rejected the timestamp request (status " + statusInt + (statusString ? ": " + statusString : "") + ")");
      }
      const tokenTlv = parts[1];
      if (!tokenTlv) throw new Error("TSA returned no timestamp token");
      if (tokenTlv.tag === 0x30) return _derRawOf(tokenTlv); // bare ContentInfo
      if (tokenTlv.tag === 0xa0) {
        // [0] IMPLICIT ContentInfo: the A0 content IS the ContentInfo's
        // content — re-wrap it in the SEQUENCE tag
        return new Uint8Array(_d.tlv(0x30, [...tokenTlv.value]));
      }
      throw new Error("Unexpected timestamp token tag: 0x" + tokenTlv.tag.toString(16));
    },

    /** POST a TimeStampReq to an RFC 3161 TSA and return the token.
        url: the TSA endpoint (http:// or https://). Returns the token DER. */
    async requestTimestamp(url, messageImprint, timeoutMs) {
      if (!global.fetch) throw new Error("Timestamps need fetch() — not available here");
      const req = this.buildTimestampRequest(messageImprint);
      const ctrl = new AbortController();
      const timer = setTimeout(() => ctrl.abort(), timeoutMs || 15000);
      let res;
      try {
        res = await global.fetch(String(url), {
          method: "POST",
          headers: {
        ...
        throw new Error("Timestamp authority unreachable: " + (e.name === "AbortError" ? "timeout" : (e.message || e)));
      } finally {
        clearTimeout(timer);
      }
      if (!res.ok) throw new Error("Timestamp authority HTTP " + res.status);
      const body = new Uint8Array(await res.arrayBuffer());
      return this.parseTimestampResponse(body);
    },
    ...
    /** Build a detached PKCS#7 SignedData over a content digest.
        ...
        tsaUrl (RFC 3161 TSA endpoint — embeds a signatureTimeStamp attribute
        so the signature stays valid after the cert expires), tsToken (a
        pre-obtained TimeStampToken, for hermetic tests) }
    ...
    async buildCms(messageDigest, opts) {
      ...
      // signed attributes. DER requires a SET OF to be sorted by the full
      // encoding of each element (byte-wise, length byte included): contentType
      // (0x18) < signingTime (0x1c) < messageDigest (0x2f), and the timestamp
      // attribute (30 82 …) sorts last. A verifier that re-encodes the SET in
      // DER order (BouncyCastle) will fail the signature if the bytes are in
      // any other order.
      ...
      const baseItems = [attrContentType, attrSigningTime, attrMessageDigest].flat();
      let attrItems = baseItems;

      if (opts.tsaUrl || opts.tsToken) {
        // PAdES signatureTimeStamp (ETSI TS 102 778): the token's
        // messageImprint is the hash of the signed attributes EXCLUDING the
        // timestamp attribute; the token is then added and the signer signs
        // the FULL attribute set. A verifier that accepts the token treats
        // the signature as made at the token's genTime, so it stays valid
        // after the certificate expires.
        const baseSet = _d.tlv(0x31, baseItems);
        const imprint = new Uint8Array(await subtle.digest("SHA-256", new Uint8Array(baseSet)));
        const token = opts.tsToken
          ? new Uint8Array(opts.tsToken)
          : await this.requestTimestamp(opts.tsaUrl, imprint);
        const attrTs = _d.seq(_d.oid(OID.signatureTimeStamp), _d.setOf(...[...token]));
        attrItems = [...baseItems, ...attrTs];
      }

      // the SET OF Attribute DER (tag 0x31) is the SIGNATURE INPUT (RFC 5652
      // §5.4: the IMPLICIT [0] tag is not included in the signed data)
      const signedAttrsSet = _d.tlv(0x31, attrItems);

      // sign the DER of the signed attrs SET with the private key
      const key = await subtle.importKey("pkcs8", opts.key, { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, false, ["sign"]);
      const sig = new Uint8Array(await subtle.sign({ name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, key, new Uint8Array(signedAttrsSet)));

      // issuerRaw/serialRaw are Uint8Arrays — spread them into plain arrays:
      // tlv's .flat() does NOT flatten typed arrays, and a Uint8Array element
      // would coerce to NaN→0 bytes, silently zeroing the signer id (a
      // stricter validator then rejects the CMS).
      const signerInfo = _d.seq(
        _d.int([1]),
        _d.seq([...certInfo.issuerRaw], [...certInfo.serialRaw]),
        _d.alg(OID.sha256),
        // signedAttrs is [0] IMPLICIT SignedAttributes (RFC 5652 §5.3) — the
        // A0 content IS the attributes, no nested SET tag. (An EXPLICIT
        // `A0 31 …` form makes BouncyCastle read the SET as an attribute.)
        _d.tlv(0xa0, attrItems),
        _d.alg(OID.sha256WithRSA),
        _d.oct(sig),
      );

      const certs = [opts.signerCert, ...(opts.chain || [])];
      // certificates is [0] IMPLICIT CertificateSet — the A0 content is the
      // cert SEQUENCEs directly, again with no nested SET tag
      const certSet = _d.tlv(0xa0, certs.map((c) => [...c]).flat());
    ...
    async signPdf(pdfBytes, opts) {
      const o = opts || {};
      const { PDFDocument, PDFName, PDFHexString, PDFString } = global.PDFLib;
      ...
      const sigRef = ctx.register(ctx.obj({
        ...
        // /M and /Reason must be PDF *strings* (the spec says M is a date
        // string) — bare JS strings would serialize as /Name tokens, which
        // Acrobat and PDFBox reject or ignore
        M: PDFString.of("D:" + this._pdfDate(o.time || new Date())),
        Reason: PDFString.of(String(o.reason || "Signed in Volt")),
        ByteRange: [0, 0, 0, 0],
    ...
    /** Fill the ByteRange + Contents placeholders. The placeholder file has
        ...
        (padded to the run width) goes into the excluded gap. The excluded
        region spans the FULL `<hex>` value — delimiters included — which is
        what Acrobat's own signatures declare and what PDFBox's /Contents
        extraction expects (it hex-decodes the region from its second byte). */
    async _patchSignature(saved, o, pfx, hexCap) {
      ...
      // the excluded region is the whole `<hex>` value: from the `<` (lt)
      // through the `>` (hexEnd + 1), delimiters included (the Acrobat /
      // PDFBox convention — see the doc comment above)
      const a0 = lt, b0 = hexEnd + 1;
      const a = a0 + DELTA, b = b0 + DELTA, nf = n + DELTA;

      // ByteRange is [start1 length1 start2 length2] — the FOURTH element is
      // the length of the covered tail (nf − b), NOT the end offset nf. A
      // range that runs past EOF is malformed and Acrobat/PDFBox flag it.
      const brFinal = "[0 " + [a, b, nf - b].map((v) => String(v).padStart(10, "0")).join(" ") + "]";
      ...
      const cms = await this.buildCms(digest, {
        ...
        tsaUrl: o.tsaUrl, tsToken: o.tsToken,
      });
      let hex = "";
      ...
      // the hex run goes right after the `<` (at a + 1); the `<` and the `>`
      // themselves stay in the excluded gap
      write(a + 1, hex + "0".repeat(pad));
      return out;
    },
