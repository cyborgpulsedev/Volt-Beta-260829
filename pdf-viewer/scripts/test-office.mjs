...
// ── XML validity of the generated packages ─────────────────────
// Two shipped bugs got past every structural test above because nothing ever
// asked "is this actually XML?": (1) pdf.js hands back raw glyph codes for
// symbolically-encoded fonts, so control bytes landed in document.xml and Word
// refused to open the file at all; (2) a theme literal wrote "<a:marL=…" —
// attributes as an element name — breaking EVERY exported .pptx.
const CTRL = /[\0-\x08\x0B\x0C\x0E-\x1F\x7F]/;
const BAD_ELEMENT = /<\/?[A-Za-z_][\w.:-]*=/; // an element NAME can never contain '='
function scanXml(entries, label) {
  const parts = entries.filter((e) => e.name.endsWith(".xml") || e.name.endsWith(".rels"));
  t(label + ": package has xml parts", parts.length > 0);
  const ctrl = parts.filter((e) => CTRL.test(str(e.data))).map((e) => e.name);
  if (ctrl.length) console.log("      control chars in: " + ctrl.join(", "));
  t(label + ": no illegal XML control characters", ctrl.length === 0);
  const badEl = parts.filter((e) => BAD_ELEMENT.test(str(e.data))).map((e) => e.name);
  if (badEl.length) console.log("      attribute-as-element in: " + badEl.join(", "));
...
t("OE.xml keeps tab/newline and escapes entities", OE.xml("a\t<b>&\n") === "a\t&lt;b&gt;&amp;\n");
t("OE.xml drops an unpaired surrogate", OE.xml("a\uD800b") === "ab");
t("OE.xml keeps a valid astral pair", OE.xml("a\uD83D\uDE00b") === "a\uD83D\uDE00b");
t("docx: a control char in page text yields clean XML", (() => {
  const d = OE.docx({ title: "t", pages: [{ num: 1, paragraphs: [{ text: "bad\u0001\u001Etext" }], tables: [], images: [] }] });
  const doc = str(readZip(d).find((e) => e.name === "word/document.xml").data);
  return !CTRL.test(doc) && doc.includes("badtext");
})());

// wp:docPr/@id and pic:cNvPr/@id are ST_DrawingElementId (xsd:unsignedInt).
// Emitting the relationship id there ("rIdImg1") made Word refuse to open ANY
// export of a PDF that contained an image, while text-only exports were fine.
t("docx: drawing element ids are numeric", (() => {
  const png = Uint8Array.from([0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A]);
  const d = OE.docx({ title: "img", pages: [{ num: 1, paragraphs: [], tables: [],
    images: [{ bytes: png, mime: "image/png", wPts: 100, hPts: 50 }] }] });
  const doc = str(readZip(d).find((e) => e.name === "word/document.xml").data);
  const ids = [...doc.matchAll(/<(?:wp:docPr|pic:cNvPr) id="([^"]*)"/g)].map((m) => m[1]);
  return ids.length === 2 && ids.every((v) => /^\d+$/.test(v) && Number(v) > 0);
})());
