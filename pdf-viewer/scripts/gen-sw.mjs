...
// Navigations (the app shell — index.html and its inline boot scripts) are
// NETWORK-FIRST with a cache fallback: the shell is always fetched fresh when
// online, so an obsolete cached bundle can never keep serving a stale UI
// (missing features, an outdated settings modal) — the recurring "I restarted
// but still see the old program" report on both desktop and PWA. Only offline
// do we fall back to the cached shell. Everything else is content-hashed and
// immutable, so it stays CACHE-FIRST: a versioned asset is fetched from
// cache with no network round-trip and only precached once.
