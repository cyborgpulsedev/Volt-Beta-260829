...
- **A tidy toolbar** — secondary actions are grouped under three dropdowns:
  **Volt** (the logo button — Open PDF…, Open from URL…, Export / backup…, AI
  ...
  Neon / Paper / Quiet skins), and **Tools** (OCR this document, OCR
...
- **Neon / Paper / Quiet skins (View ▸ …)** — the **Neon** skin keeps the
  full cyan/violet/magenta glow on dark chrome; **Quiet** desaturates the
  accents and dials the glow down for calmer, long-form reading; **Paper**
  switches to a light background while keeping the neon accent family for
  daylight reading. Neon and Quiet invert the document (Firefox-style
  night reading): page canvas and text layer are inverted while the
  annotation overlay keeps true colors; Paper renders documents normally.
  Choice persists per user (`volt:theme`).
...
- **Exporting** — an **annotated PDF** (highlights/underlines/strikes/notes
  ...
  **Send feedback…** (the Volt ▾ menu drafts a GitHub issue on the public
  ...
  anything itself; if GitHub can't open — offline, unreachable, or the
  browser-open fails — the draft is saved as a timestamped `.md` file
  instead, so feedback is never lost), and
  **Digitally sign PDF…** (attach a real certificate signature to the
  ...
  crypto in the unit tests). An optional **RFC 3161 timestamp authority**
  (a TSA URL field in the sign dialog, remembered per user) sends only a
  hash of the signed attributes and embeds the returned token as a PAdES
  `signatureTimeStamp` attribute — the signature then stays valid after
  the certificate expires, because the TSA's token proves when it was
  made. The output is validated by a real external stack — `npm run
  test:pdfbox` signs a probe PDF through a LOCAL `openssl ts -reply` TSA
  and verifies the `/Sig` + CMS through **Apache PDFBox 3 + BouncyCastle**
  (hermetically in CI with an ephemeral openssl cert, no gitignored key
  or external network needed) — checking the ByteRange semantics, the
  signer and TSA signatures, and the timestamp's messageImprint; that
  gate caught and fixed the ByteRange length semantics, `/M` + `/Reason`
  serialized as PDF names, a zeroed signer-id, and the signed-attributes
  SET order; OpenSSL-3 PFX exports (SHA-256 MAC, PBES2 AES-256) are
  supported too. A
...
| Skin | View ▸ **Neon** / **Paper** / **Quiet** (persisted per user; Neon glows, Paper is light, Quiet mutes the glow) |
