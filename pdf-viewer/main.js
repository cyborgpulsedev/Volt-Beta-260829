...
const { app, BrowserWindow, shell, ipcMain, utilityProcess, screen, session, Menu, dialog, Tray } = require("electron");
...
const { tmpdir, totalmem } = require("node:os");
...
let win = null;
let tray = null; // system-tray icon (created once in createWindow)
...
// Every export (Word/Excel/PowerPoint/PDF-A/Markdown/backup) reaches the OS as
// a blob: download from Utils.download's <a download>. With NO will-download
// handler Electron parks the item in <guid>.tmp under the downloads dir and
// waits forever for a save path — the file is never renamed, so the user finds
...
function autoSaveDownloads() {
  session.defaultSession.on("will-download", (_e, item) => {
    try {
      ...
      for (let i = 1; existsSync(dest) && i < 1000; i++) dest = join(dir, stem + " (" + i + ")" + ext);
      item.setSavePath(dest);
      item.once("done", (_ev, state) => {
        if (win && !win.isDestroyed()) {
          win.webContents.send("volt:download-done", { ok: state === "completed", path: dest, name: basename(dest) });
        }
      });
    } catch (e) {
      console.log("will-download: " + ((e && e.message) || e)); // never break the download
    }
  });
}
  ...
  if (!SMOKE && !tray) {
    try {
      ...
      tray.setContextMenu(Menu.buildFromTemplate([
        { label: "Open Volt", click: () => { if (win) { win.show(); win.focus(); } } },
        { type: "separator" },
        { label: "Quit", click: () => app.quit() },
      ]));
      tray.on("click", () => { if (win) { win.show(); win.focus(); } });
    } catch (e) { /* tray is a nicety — never block startup on it */ }
  }
...
async function toolbarResizeStage(w) {
  ...
  const js = (code) => w.webContents.executeJavaScript(code);    const out = { ok: false, error: null, sizes: [] };
    ...
    const menuCheck = await js(`(async () => {
      ...
      out.viewHas = ["btn-fit-width", "btn-fit-page", "btn-rotate", "btn-mode-continuous", "btn-mode-one", "btn-mode-spread", "btn-theme-neon", "btn-theme-paper", "btn-theme-quiet"].every(has);
      ...
      aria.menuitems = ["btn-open", "btn-open-url", "btn-export", "btn-menu-settings", "btn-menu-help",
        ...
        "btn-fit-width", "btn-fit-page", "btn-rotate", "btn-mode-continuous", "btn-mode-one", "btn-mode-spread", "btn-theme-neon", "btn-theme-paper", "btn-theme-quiet",
      ...
      document.dispatchEvent(kd({ key: "ArrowDown" })); // → Neon skin
      document.dispatchEvent(kd({ key: "ArrowDown" })); // → Paper skin
      document.dispatchEvent(kd({ key: "ArrowDown" })); // → Quiet skin
      ...
      out.kbWrapUp = focusedId() === "btn-theme-quiet";
      document.dispatchEvent(kd({ key: "Home" }));
      document.dispatchEvent(kd({ key: "End" }));
      out.kbHomeEnd = focusedId() === "btn-theme-quiet";
          ...
          if (wraps >= 1 && textSpans > 0 && (!EXPECT_DOC || docName === EXPECT_DOC)) {
            ...
            try {
              ...
              try {
                ...
                // the recommendation is sized to the machine's RAM, so the model
                // offered here is 1.7b / 4b / 8b depending on the host. Ask the
                // app which one it picked rather than pinning a tier the probe
                // would only pass on an 8 GB box.
                const wantBm = AI._bootstrapModel();
            ...
            // ── Send-feedback offline/local fallback ───────────
            // the modal submit must never lose the draft: when the machine is
            // offline, github.com is unreachable, or the external open throws,
            // _sendFeedback saves the report as a local .md file instead of
            // opening a dead browser tab. Exercise all four paths with stubs.
            // A spy on _openExternal guarantees paths 1-3 never leak a real
            // browser tab (noLeakedTab).
            const feedbackProbe = { error: null };
            let realOpenExternalCalled = false;
            try {
              const A = Volt.App;
              const fbEl = A.elements;
              const setDraft = (t) => {
                fbEl.feedbackType.value = "Bug report";
                fbEl.feedbackText.value = t;
                fbEl.feedbackEmail.value = "";
              };
              const origDownload = Utils.download;
              const origToast = A.toast;
              const origFetch = window.fetch;
              const origOpenExternal = A._openExternal;
              const captured = { name: null, blob: null, url: null, toasts: [] };
              Utils.download = (b, n) => { captured.blob = b; captured.name = n; };
              A.toast = (m) => { captured.toasts.push(m); };
              ...
              const blobText = async () => (captured.blob instanceof Blob) ? await captured.blob.text() : "";
              const reset = () => { captured.name = null; captured.blob = null; captured.url = null; captured.toasts = [];
              };
              // Wrap the real _openExternal so we can verify it's never called
              // during the offline/unreachable/throw paths (paths 1-3).
              A._openExternal = async function (url) {
                realOpenExternalCalled = true;
                throw new Error("must not leak a real external-open tab during smoke probe");
              };
              // 1) offline fast path — navigator.onLine false → saved as a file
              setOnLine(false);
              setDraft("Offline probe — must not open a browser tab");
              reset();
              await A._sendFeedback();
              const fbT1 = await blobText();
              feedbackProbe.offlineSaved = !!captured.name && captured.name.endsWith(".md") &&
                fbT1.indexOf("Offline probe — must not open a browser tab") !== -1 &&
                fbT1.indexOf("github.com") !== -1 &&
                captured.toasts.some((m) => m.indexOf("Offline — feedback saved as") === 0);
              // 2) online but github.com unreachable — the probe rejects
              setOnLine(true);
              window.fetch = () => Promise.reject(new TypeError("network down"));
              setDraft("Unreachable probe — fetch rejects");
              reset();
              await A._sendFeedback();
              const fbT2 = await blobText();
              feedbackProbe.unreachableSaved = !!captured.name && captured.name.endsWith(".md") &&
                fbT2.indexOf("Unreachable probe — fetch rejects") !== -1 &&
                captured.toasts.some((m) => m.indexOf("Can't reach GitHub — feedback saved as") === 0);
              // 3) probe ok but the external open throws → saved as a file
              window.fetch = () => Promise.resolve(new Response(null, { status: 200 }));
              A._openExternal = async () => { throw new Error("no external browser"); };
              setDraft("Open-throw probe — external open fails");
              reset();
              await A._sendFeedback();
              const fbT3 = await blobText();
              feedbackProbe.openThrewSaved = !!captured.name && captured.name.endsWith(".md") &&
                fbT3.indexOf("Open-throw probe — external open fails") !== -1 &&
                captured.toasts.some((m) => m.indexOf("Couldn't open GitHub — feedback saved as") === 0);
              // 4) happy path — GitHub reachable, no file, issue URL opened
              A._openExternal = async (u) => { captured.url = u; };
              setDraft("Happy probe — opens the issue page");
              reset();
              await A._sendFeedback();
              feedbackProbe.openedToGithub = captured.name === null && captured.blob === null &&
                (captured.url || "").indexOf("https://github.com/cyborgpulsedev/Volt-Beta-260829/issues/new?labels=") === 0 &&
                (captured.url || "").indexOf(encodeURIComponent("Happy probe — opens the issue page")) !== -1 &&
                captured.toasts.some((m) => m.indexOf("review & submit on GitHub") !== -1);
              // restore the real environment
              Utils.download = origDownload;
              A.toast = origToast;
              window.fetch = origFetch;
              A._openExternal = origOpenExternal;
              if (onLineDesc) Object.defineProperty(window.navigator, "onLine", onLineDesc);
              else delete window.navigator.onLine;
            } catch (e) { feedbackProbe.error = String((e && e.message) || e); }
            // Smoke: the real external-open must never be called during feedback probes;
            // paths 1-3 must fall back to a local file, never leak a browser tab.
            feedbackProbe.noLeakedTab = realOpenExternalCalled === false;
            feedbackProbe.allOk = !feedbackProbe.error &&
              feedbackProbe.offlineSaved === true &&
              feedbackProbe.unreachableSaved === true &&
              feedbackProbe.openThrewSaved === true &&
              feedbackProbe.openedToGithub === true &&
              feedbackProbe.noLeakedTab === true;
            ...
            try {
              ...
              // force-play the first-run offer; clicking it opens the Volt
              // logo menu and focuses the setup wizard item
              ...
              // prefers-reduced-motion: the CSS has a media query that sets
              // transition: none on the hint, collapsing the grow/shrink into
              // an instant appearance/disappearance. Verify the rule exists in
              // any loaded stylesheet.
              const tick = () => new Promise((r) => setTimeout(r, 60));
              let foundReducedRule = false;
              for (const ss of document.styleSheets) {
                try {
                  for (const rule of ss.cssRules || []) {
                    if (rule.conditionText && rule.conditionText.indexOf("prefers-reduced-motion") >= 0) {
                      for (const sub of rule.cssRules || []) {
                        if (sub.selectorText && sub.selectorText.indexOf("#start-hint") >= 0 &&
                            sub.style && sub.style.transition === "none") {
                          foundReducedRule = true;
                          break;
                        }
                      }
                    }
                    if (foundReducedRule) break;
                  }
                } catch (e) { /* cross-origin sheet */ }
                if (foundReducedRule) break;
              }
              setup.hintFadesWhenReduced = foundReducedRule;
              S._maybeShowStartHint(true);
              await tick();
              // the hint is a normal flex child — no absolute positioning
              setup.hintSlidesByDefault = getComputedStyle(hint).position !== "absolute" &&
                hint.offsetWidth > 0 && hint.hidden === false;
              hint.click();
              await tick();
              // clicking opens the Volt logo menu with the setup wizard focused
              setup.hintOpensHelp = document.getElementById("menu-brand-panel").hidden === false;
              // close the menu so the rest of the test runs clean
              document.getElementById("btn-brand").click();
              ...
              setup.opens = sModal.hidden === false &&
                ...
                document.querySelectorAll(".setup-track-seg").length === 4;
              ...
              setup.desktopOption = setupDesktopBridge
                ? document.getElementById("setup-desktop-opt").hidden === false &&
                  ...
                  document.querySelectorAll('input[name="setup-skin"]').length === 3
              ...
              // finish: desktop task unchecked (no real script), neon skin default
              ...
              setup.summary = document.querySelector('.setup-step[data-step="3"]').hidden === false &&
                summary.indexOf("Skin — Neon") >= 0 && summary.indexOf("AI — qwen3:4b") >= 0;
            ...
            const clr = {};    // "Clear highlights" quick action on the conversion toast
                      const aiW = {};    // AI pane: quick row removed + stretchable width
            ...
            try {
              ...
              try {
                ...
                try {
                  ...
                  // The chat's 'Clear highlights' and 'Copy highlights' quick actions
                  // were removed on purpose when the AI footer was reworked, so the
                  // probes that drove them are gone too. Both behaviours are still
                  // covered above through the paths that remain: clr.* exercises the
                  // conversion toast and the pages manager, and aaa.copy* exercises the
                  // selection copy with citations.
                  // ── AI pane: quick row gone + stretchable width ───────
                  // the quick-prompt row is removed so the input area stays
                  // one row tall (more room to type); the highlight utility
                  // actions went with it, deliberately. The pane itself is
                  // user-stretchable via
            ...
            try {
              if (!signProbe.skipped) {
                ...
                if (!br2 || !ct2) {
                  ...
                  // certificates is [0] IMPLICIT CertificateSet — the A0
                  // content is the cert SEQUENCEs directly (no nested SET)
                  ...
                  // the signature input is the DER SET OF Attribute (0x31 +
                  // length over the implicit attr content) — RFC 5652 §5.4
                  const attrItems = signedAttrs.content;
                  const lAttrs = attrItems.length;
                  const lenB = lAttrs < 0x80 ? [lAttrs] : lAttrs < 0x100 ? [0x81, lAttrs] : [0x82, (lAttrs >> 8) & 0xff, lAttrs & 0xff];
                  // tag + length-bytes + content — 1, not 2, before lenB
                  // (an extra trailing byte breaks the RSA verify)
                  const sigInput = new Uint8Array(1 + lenB.length + attrItems.length);
                  sigInput[0] = 0x31; sigInput.set(lenB, 1); sigInput.set(attrItems, 1 + lenB.length);
                  signProbe.verify = await crypto.subtle.verify(
                    { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, vk, signature.content, sigInput);
                  let mdOk = false;
                  // attributes are directly in the A0 content (implicit)
                  for (const attr of kidsOf(signedAttrs.content)) {
  ...
  app.whenReady().then(async () => {
    try {
      ...
      ipcMain.handle("volt:app-info", () => ({
        ...
        updaterEnabled,
        // total RAM in GB — the renderer sizes its local-model recommendation
        // from this (navigator.deviceMemory caps at 8, useless above that)
      ...
      autoSaveDownloads(); // exports land in Downloads under their real name
